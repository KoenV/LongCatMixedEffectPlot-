...
# function
LongCatMixedEffectPlot = function(fit,conf.level=.95,dodge.level=.70,n.placement=1.5){
  ...
  n_table$placement = range(df_with_info$y)[1] - (range(df_with_info$y)[2] - range(df_with_info$y)[1])/n.placement

  
  # plotting
  # get labels for plot
  plot_labels = list(
    x = ifelse(label(fit@frame)[2]=='',names(fit@frame)[2],label(fit@frame)[2]),
  ...
  ggplot() + 
    ...
    geom_linerange(data=df_with_info,aes(ymin=lower_ci,ymax=upper_ci,x=moment.cont,group=condition),
                   alpha=.5,position=position_dodge(width = dodge.level)) + scale_x_discrete(limits=df_with_info$moment) +
